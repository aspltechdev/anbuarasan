...
function About() {
  ...
  const calcParallax = (depth) => {
    ...
    const rect = sectionRef.current.getBoundingClientRect();
    return {
      transform: `translate3d(${(mousePos.x - rect.left - rect.width / 2) * depth}px, ${
        (mousePos.y - rect.top - rect.height / 2) * depth
      }px, 0)`,
    };
  };
      ...
      <div className="about-container">
        ...
        <div className={`about-cinematic-card ${isVisible ? "visible" : ""}`}>
          <div className="about-card-inner">
            ...
            {/* ── LEFT: EduFlex mosaic (replaces about-visual-story) ── */}
            <div className="ef-mosaic" style={calcParallax(0.015)}>

              {/* Large card — top left (like EduFlex woman in orange) */}
              <div className="ef-card ef-card-lg ef-gold">
                <img
                  src="https://images.unsplash.com/photo-1545389336-cf090694435e?w=480&h=560&fit=crop&crop=face"
                  alt="Anbuarasan"
                />
                {/* Top-right arrow chip — exact EduFlex ↗ button */}
                <div className="ef-icon-chip">↗</div>
              </div>

              {/* Small card — top right (like EduFlex man in white) */}
              <div className="ef-card ef-card-sm ef-lavender">
                <img
              ...
              {/* Medium card — bottom right (like EduFlex woman with megaphone) */}
              <div className="ef-card ef-card-md ef-sand">
                <img
                  src="https://images.unsplash.com/photo-1517841905240-472988babdf9?w=360&h=300&fit=crop&crop=face"
                  alt="Wellness"
                />
                {/* Inline badge — like EduFlex link rings icon */}
                <div className="ef-badge-chip">◈◈</div>
              </div>

              {/* Hollow circle dots — EduFlex's two floating circles */}
              <div className="ef-circle ef-circle-1" />
              <div className="ef-circle ef-circle-2" />

              {/* Sparkle — EduFlex's green diamond ✦ */}
              <span className="ef-sparkle ef-sparkle-1">✦</span>
              <span className="ef-sparkle ef-sparkle-2">✦</span>
